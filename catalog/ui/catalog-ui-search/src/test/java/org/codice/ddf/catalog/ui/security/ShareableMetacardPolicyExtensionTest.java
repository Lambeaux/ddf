...
public class ShareableMetacardPolicyExtensionTest {
  ...
  private static final Permission KVP_STEVE_IS_OWNER =
      new KeyValuePermission(Core.METACARD_OWNER, ImmutableSet.of("steve"));

  private static final Permission WCP_STEVE_IS_OWNER =
      new WildcardPermission(Core.METACARD_OWNER + ":steve");

  private static final Permission WCP_ANY_OWNER =
      new WildcardPermission(Core.METACARD_OWNER + ":*");

  private static final Permission KVP_EMAILS_ABC =
      new KeyValuePermission(SecurityAttributes.ACCESS_INDIVIDUALS, ImmutableSet.of("A", "B", "C"));

  private static final Permission KVP_EMAILS_AB =
      new KeyValuePermission(SecurityAttributes.ACCESS_INDIVIDUALS, ImmutableSet.of("A", "B"));

  private static final Permission KVP_EMAILS_BC =
      new KeyValuePermission(SecurityAttributes.ACCESS_INDIVIDUALS, ImmutableSet.of("B", "C"));

  private static final Permission WCP_EMAILS_AB =
      new WildcardPermission(SecurityAttributes.ACCESS_INDIVIDUALS + ":A,B");

  private static final Permission WCP_EMAILS_BC =
      new WildcardPermission(SecurityAttributes.ACCESS_INDIVIDUALS + ":B,C");

  @Test
  public void testImplies() {
    assertThat(KVP_STEVE_IS_OWNER.implies(WCP_STEVE_IS_OWNER), is(true));
    assertThat(WCP_STEVE_IS_OWNER.implies(KVP_STEVE_IS_OWNER), is(false)); // Impl bound?!?!

    assertThat(WCP_ANY_OWNER.implies(WCP_STEVE_IS_OWNER), is(true));
    assertThat(WCP_STEVE_IS_OWNER.implies(WCP_ANY_OWNER), is(false));

    assertThat(WCP_ANY_OWNER.implies(KVP_STEVE_IS_OWNER), is(false)); // Impl bound?!?!
    assertThat(KVP_STEVE_IS_OWNER.implies(WCP_ANY_OWNER), is(false));

    assertThat(KVP_EMAILS_AB.implies(WCP_EMAILS_AB), is(true));
    assertThat(WCP_EMAILS_AB.implies(KVP_EMAILS_AB), is(false)); // Impl bound?!?!
    assertThat(KVP_EMAILS_BC.implies(WCP_EMAILS_BC), is(true));
    assertThat(WCP_EMAILS_BC.implies(KVP_EMAILS_BC), is(false)); // Impl bound?!?!

    assertThat(KVP_EMAILS_AB.implies(KVP_EMAILS_BC), is(false));
    assertThat(KVP_EMAILS_AB.implies(WCP_EMAILS_BC), is(false));
    assertThat(KVP_EMAILS_ABC.implies(KVP_EMAILS_ABC), is(true));
    assertThat(KVP_EMAILS_ABC.implies(KVP_EMAILS_AB), is(true));
    assertThat(KVP_EMAILS_ABC.implies(KVP_EMAILS_BC), is(true));
    assertThat(KVP_EMAILS_ABC.implies(WCP_EMAILS_AB), is(true));
    assertThat(KVP_EMAILS_ABC.implies(WCP_EMAILS_BC), is(true));
  }
