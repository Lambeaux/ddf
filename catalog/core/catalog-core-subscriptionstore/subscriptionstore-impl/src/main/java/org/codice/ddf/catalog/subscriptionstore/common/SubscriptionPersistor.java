...
public class SubscriptionPersistor {
    ...
    private static final String SUBSCRIPTION_SERIALIZED_FILTER = "filter";

    private static final String SUBSCRIPTION_CALLBACK_URL = "callback";
     ...
     *
     * @return a map of subscription id's to their accompanying metadata.
    ...
    /**
     * Add the subscription metadata to the persistent store using it's id for the document key.
     * <p>
     * Insertion operations with the same key overwrite the previous value, which means a duplicate
     * being added can be treated as a no-op, minus the network overhead.
     *
    ...
    private SubscriptionMetadata mapToMetadata(Map<String, Object> map) {
        notEmpty(map, "persistentItem cannot be null or empty");

        String id = (String) map.get(SUBSCRIPTION_ID_SOLR);
        String type = (String) map.get(SUBSCRIPTION_TYPE);
        String serializedSubscription = (String) map.get(SUBSCRIPTION_SERIALIZED_FILTER);
        String callback = (String) map.get(SUBSCRIPTION_CALLBACK_URL);
    ...
    private String getEcqlStringForId(String subscriptionId) {
        return format("\"%s_txt\" LIKE '%s'", SUBSCRIPTION_ID_SOLR, subscriptionId);
    }
}